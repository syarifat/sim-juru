@php
    ...
    $linkClass = "group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors hover:bg-blue-600 hover:text-white text-gray-300 mx-2 mb-1";
...
<nav class="space-y-1">
    ...
    <a href="{{ route('dashboard') }}" class="{{ $linkClass }} {{ request()->routeIs('dashboard') ? 'bg-blue-600 text-white' : '' }}">
    ...
    @if($role === 'Admin')
        ...
        <a href="{{ route('admin.users.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.users.*') ? 'bg-blue-600 text-white' : '' }}">
        ...
        <a href="{{ route('admin.tahun-ajaran.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.tahun-ajaran.*') ? 'bg-blue-600 text-white' : '' }}">
        ...
        <a href="{{ route('admin.kelas-mapel.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.kelas-mapel.*', 'admin.kelas.*', 'admin.mapel.*') ? 'bg-blue-600 text-white' : '' }}">
        ...
        <a href="{{ route('admin.jam-pelajaran.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.jam-pelajaran.*') ? 'bg-blue-600 text-white' : '' }}">
        ...
        <a href="{{ route('admin.jadwal.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.jadwal.*') ? 'bg-blue-600 text-white' : '' }}">
    ...
    @if($role === 'Guru')
        ...
        <a href="{{ route('guru.jurnal.index') }}" class="{{ $linkClass }} {{ request()->routeIs('guru.jurnal.index', 'guru.jurnal.create') ? 'bg-blue-600 text-white' : '' }}">
        ...
        <a href="{{ route('guru.jurnal.riwayat') }}" class="{{ $linkClass }} {{ request()->routeIs('guru.jurnal.riwayat') ? 'bg-blue-600 text-white' : '' }}">
    ...
    @if($role === 'Kepala_Sekolah')
        ...
        <a href="{{ route('kepsek.validasi.index') }}" class="{{ $linkClass }} {{ request()->routeIs('kepsek.validasi.*') ? 'bg-blue-600 text-white' : '' }}">
    ...
    <div class="md:hidden mt-8 border-t border-slate-800 pt-4 px-2 pb-6">
        <div class="px-3 mb-2">
            <p class="text-sm font-semibold text-white">{{ Auth::user()->username ?? 'Guest' }}</p>
            <p class="text-xs text-slate-400">{{ str_replace('_', ' ', Auth::user()->role ?? '') }}</p>
        </div>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full text-left group flex items-center px-3 py-2 text-sm font-medium rounded-lg hover:bg-red-900 text-red-400 transition-colors">
                <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                <span>Logout</span>
            </button>
        </form>
    </div>
