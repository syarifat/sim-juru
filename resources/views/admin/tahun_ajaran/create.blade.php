    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Formulir penambahan data tahun ajaran baru.</p>
            </div>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.tahun-ajaran.store') }}" method="POST" class="p-6 md:p-8 space-y-6">
                @csrf
                
                <div class="space-y-4">
                    <div>
                        <label for="tahun" class="block text-sm font-semibold text-gray-700 mb-1">Tahun Ajaran <span class="text-red-500">*</span></label>
                        <input type="text" name="tahun" id="tahun" value="{{ old('tahun') }}" placeholder="Contoh: 2023/2024" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-shadow @error('tahun') border-red-500 @enderror" required>
                        @error('tahun')
