        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100" x-data="{ currentRole: '{{ old('role', $user->role) }}' }">
            <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" class="w-full text-sm border border-gray-300 rounded-lg focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div x-show="currentRole !== 'Admin'" x-transition:enter="transition ease-out duration-200" class="space-y-5 bg-gray-50/70 p-4 rounded-lg border border-gray-100">
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $user->guru->nama_lengkap ?? '') }}" class="w-full text-sm border border-gray-300 bg-white rounded-lg focus:border-emerald-500 focus:ring-emerald-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $user->guru->nip ?? '') }}" class="w-full text-sm border border-gray-300 bg-white rounded-lg focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="password" name="password" id="password" placeholder="Minimal 8 karakter" class="w-full text-sm border border-gray-300 rounded-lg focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="pt-4 flex justify-end space-x-3 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
